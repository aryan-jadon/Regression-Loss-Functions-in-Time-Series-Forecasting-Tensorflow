...
import libs.tft_model_rmsle_loss
...
ModelClass = libs.tft_model_rmsle_loss.TemporalFusionTransformer
tf.experimental.output_all_intermediates(True)

with open('favorita_dataset_experiments.json', 'r') as f:
    loss_experiment_tracker = json.load(f)

dataset_name = "favorita"
dataset_folder_path = "favorita_dataset"
...
loss_experiment_tracker.update({
    "Root Mean Squared Logarithmic Error p10 Loss": str(p10_loss.mean()),
    "Root Mean Squared Logarithmic Error p50 Loss": str(p50_loss.mean()),
    "Root Mean Squared Logarithmic Error p90 Loss": str(p90_loss.mean()),
})

with open("favorita_dataset_experiments.json", "w") as outfile:
